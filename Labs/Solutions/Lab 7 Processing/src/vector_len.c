...
int main(int argc, char** argv) {
    ...
    double *local_sums = alloc_vec(world_size);
    MPI_Gather(&local_len, 1, MPI_DOUBLE, local_sums, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    if(world_rank == 0) {
        total = compute_sum(local_sums, world_size);
    }
