...
Buffer *copy_buffer(Buffer *buffer) {
    ...
    // note: The string libary only calculates up to '\0' and mem lib does till EOF
    // malloc (twice, one for the struct and one for the data)
    // memset 
    Buffer *cpyBuff = malloc(sizeof(Buffer));
    cpyBuff->data = malloc(sizeof(char) * buffer->size);

    cpyBuff->size = buffer->size;
    memcpy(cpyBuff->data, buffer->data, buffer->size);

    // OR
    // for (int i = 0; i < buffer->size; i++) 
    //     memset(&cpyBuff->data[i], buffer->data[i], sizeof(char));
