...
struct Vector {
    double *data;  // Need to malloc this as well
    int size;
};
...
{
    // Allocating space to pointers
    struct Vector *newVec = malloc(sizeof(struct Vector));
    newVec->data = malloc(sizeof(double) * n);  // have to do this

    memset(newVec->data, 0, sizeof(double) * n);
    newVec->size = n;
...
struct Vector *add_vectors(struct Vector *v1, struct Vector *v2) 
{
    ...
    struct Vector *vecSum = malloc(sizeof(struct  Vector));

    vecSum->size = v1->size;
    vecSum->data = malloc(sizeof(double) * v1->size);
    
    for (int i = 0; i < v1->size; i++) {
        vecSum->data[i] = v1->data[i] + v2->data[i];
    }
